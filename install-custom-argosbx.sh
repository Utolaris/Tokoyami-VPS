...
# 内嵌 Argosbx 上游脚本版本的校验值；运行时不再下载上游主脚本。
EMBEDDED_ARGOSBX_SHA256="7377af697edea4eed4df422c136f13b5675a333626d7dcbd442bcbf5ea77cf47"
...
# 将内嵌的 Argosbx 上游脚本写入临时文件。
# 这一步只使用当前脚本自带内容，不再从 GitHub raw 拉取上游主脚本。
write_embedded_upstream() {
  local tmp="${TMPDIR:-/tmp}/argosbx-embedded.sh"
  cat > "$tmp" <<'__TOKOYAMI_EMBEDDED_ARGOSBX_EOF__'
# Embedded upstream Argosbx script from https://github.com/yonggekkk/argosbx
# Original license: GPL-3.0. Tokoyami-VPS stores this copy to avoid runtime raw-script downloads.
#!/bin/sh
...
agsbxurl="https://raw.githubusercontent.com/Utolaris/Tokoyami-VPS/main/install-custom-argosbx.sh"
showmode(){
echo "Argosbx脚本一键SSH命令生器在线网址：https://yonggekkk.github.io/argosbx/"
echo "主脚本：curl -fsSL https://raw.githubusercontent.com/Utolaris/Tokoyami-VPS/main/install-custom-argosbx.sh | bash"
...
mkdir -p "$HOME/bin"
# Tokoyami-VPS embeds Argosbx, so persist this local script instead of downloading from GitHub raw.
if [ -f "$0" ]; then
cp "$0" "$SCRIPT_PATH"
else
echo "Cannot persist embedded agsbx script: $0 not found" >&2
exit 1
fi
chmod +x "$SCRIPT_PATH"
...
__TOKOYAMI_EMBEDDED_ARGOSBX_EOF__
  chmod +x "$tmp"
  ...
  if command -v sha256sum >/dev/null 2>&1; then
    ...
    actual_sha="$EMBEDDED_ARGOSBX_SHA256"
  fi

  if [ "$actual_sha" != "$EMBEDDED_ARGOSBX_SHA256" ]; then
    echo "Embedded Argosbx checksum mismatch." >&2
    exit 1
  fi
...
install_argosbx_base() {
  local upstream
  upstream="$(write_embedded_upstream)"
